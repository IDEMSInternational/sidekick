...
function mapOutArticleWithSubItems(body,startPosition,numberOfElements,article_Name,sectionTypes){
  var sectionTables = [];
  var subItems = [];
  var map = {articleName: article_Name,startElement: null,end:-1, articleTable: null, subItems: subItems, cleanUpElements: []};
  ...
  for (var i = startPosition; i < numberOfElements; i++){
    ...
    if (isSubItemHeader(element)){
      if (map.startElement == null) {
        map.startElement = element;
      }
      if (element.getText().trim() != '') /// easy to have a blank header. should use last section if it is the case
      {
        var subItem = {header:element,sectionTables:JSON.parse(JSON.stringify(sectionTables))} //add clone of sectionTable
        var sectionToUpdate = null;   
        map.subItems.push(subItem);
      }
      else {
        map.cleanUpElements.push(element)
      }
      ...
      {
        var sectionTypeName = element.getText();
        var sectionToUpdate = subItem.sectionTables.find(function(section) {
          return section.sectionName === sectionTypeName;
        });
        if (sectionToUpdate) {
          sectionToUpdate.headers.push(element);
